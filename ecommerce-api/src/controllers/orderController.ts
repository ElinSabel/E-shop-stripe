...
const createOrderItem = async (data: IOrderItem) => {
  const {order_id, product_id, product_name, product_image ,quantity, unit_price} = data;
  try {
    const sql = `
    ...
    `;
    const params = [order_id, product_id, product_name, product_image, quantity, unit_price]
